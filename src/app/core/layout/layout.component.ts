...
// import menuItems from '../../data/menu-items.json'; // Commented out - now using API
...
export class LayoutComponent implements OnInit {
  ...
  // menuItems: MenuItem[] = menuItems as MenuItem[]; // Commented out - now using API
  menuItems: MenuItem[] = [];
  ...
  ngOnInit(): void {
    this.authState$ = this.authService.authState$.subscribe((state: any) => {
      ...
      console.log('Layout - User roleid:', this.currentUser?.roleid);
      
      // Load menus from API based on user's role ID
      if (this.currentUser?.roleid) {
        this.loadMenusFromAPI(this.currentUser.roleid);
      } else {
        console.warn('No roleid found for user, cannot load menus');
        this.menuItems = [];
  ...
  loadMenusFromAPI(roleId: string): void {
    console.log('Loading menus from API for roleId:', roleId);
    this.menuLoading = true;
    this.apiService.getMenusByRole(roleId).subscribe({
      next: (response: any) => {
        ...
        console.log('Menus loaded from API:', this.menuItems);
        this.filteredMenuItems = this.menuItems; // API already filters by role
  ...
  // Commented out - API already returns filtered menus by role
  ...
  toggleSubmenu(item: MenuItem): void {
    // Support both API structure (submenu) and old JSON structure (children)
    const hasSubitems = (item.submenu && item.submenu.length > 0) || (item.children && item.children.length > 0);
    if (hasSubitems) {
